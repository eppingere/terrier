...
void StringFunctions::Substring(UNUSED_ATTRIBUTE exec::ExecutionContext *ctx, StringVal *result, const StringVal &str,
  ...
  const auto str_start = std::max(pos.val_ - 1, static_cast<int64_t>(0));
  const auto str_len = std::min(static_cast<int64_t>(str.len_) - str_start, len.val_);
  ...
  // All good
  *result = StringVal(str.Content() + str_start, uint32_t(str_len));
}
