...
BENCHMARK_DEFINE_F(TBBBENCHMARK, NUMAThreadPoolBasicNoAllocation)(benchmark::State &state) {
  // 500 MB array.
  std::vector<uint8_t> array(500 * 1024 * 1024);
  ...
  // Num threads from arguments.
  const uint32_t num_threads = state.range(0);
  std::function<void()> queue[num_threads];
  std::atomic<double> total = 0.0;
  for (uint64_t thread_id = 0; thread_id < static_cast<uint64_t>(num_threads); thread_id++) {
  ...
  std::vector<int> cpus;
  for (uint32_t i = 0; i < num_threads; i++) cpus.push_back(i);
  common::ExecutionThreadPool pool(common::ManagedPointer(&registry), &cpus);

  for (auto _ : state) {
    // Create thread pool.

    std::promise<void> ps[num_threads];

    for (uint32_t i = 0; i < num_threads; i++) {
      pool.SubmitTask(&ps[i], queue[i]);
    }

    for (uint32_t i = 0; i < num_threads; i++) {
      ps[i].get_future().wait();
    }
...
BENCHMARK_REGISTER_F(TBBBENCHMARK, NUMAThreadPoolBasicNoAllocation)->Apply(CustomArguments)->Iterations(50)->Unit(benchmark::kMillisecond);
